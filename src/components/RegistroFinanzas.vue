<template>
    <div>
      ...
      <div v-for="(registro, index) in registros" :key="registro.id" :class="registro.tipo">
        ...
        <p v-if="index !== editIndex">{{ registro.descripcion }}: ${{ registro.monto }} (<span v-format-date="registro.fecha"></span>)</p>
        ...
        <button @click="editRecord(index)">Editar</button>
        <button @click="deleteRecord(index)">Eliminar</button>
      </div>
